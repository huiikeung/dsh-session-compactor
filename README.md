...
- **点击圆环 = 官方 breakdown 弹窗照常弹出**（system/tools/messages 分布，
  和点其它官方组件一样），**同时**在圆环**右侧**展开我们的两个按钮
...
- 按钮与行内 pill/圆环**同高度同视觉语言**（line-height 20px + padding 1px 的
  22px 胶囊，无外框无底色，hover 才出底），展开**不撑高** dock 行；
- 再点一次圆环，弹窗与按钮一起收起；点页面别处按钮收起（官方弹窗按它
  自己的规则处理）；
- agent 运行中按钮自动禁用；全新空白会话不显示。

### 5.1 设置侧栏分页「上下文压缩」（0.6.8 起为独立设置页）
...
2. 输入框下方官方「上下文用量」圆环保持不变，点击圆环即在它右侧展开「压缩总结」「提示增强」（按钮与行内 pill 同高度，不撑高行）；压缩设置见设置侧栏「上下文压缩」分页；
